...
export default function MobilePosAppPage() {
  ...
  return (
    ...
    >
      <div className="flex flex-col text-[#1a1918] font-sans select-none space-y-6">
        {/* POS Header Bar */}
        <div className="bg-white p-6 rounded-2xl border border-[#e8e3da] shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-full border border-gold flex items-center justify-center bg-black shrink-0 shadow-sm">
              <span className="text-gold font-serif text-lg font-bold">LJ</span>
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h1 className="font-serif font-bold text-xl text-[#1a1918] tracking-wide">
              ...
              <span className="text-xs text-gray-500 block mt-0.5 font-sans">
                Operador en caja: <strong className="text-[#1a1918] font-bold">{user?.name ? user.name : 'Dueña (Adriana)'}</strong>
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <Link
              href="/admin"
              className="bg-gray-100 hover:bg-gray-200 text-gray-800 font-bold text-xs uppercase tracking-wider px-4 py-2.5 rounded-full border border-gray-300 transition-all flex items-center gap-1.5 cursor-pointer active:scale-95"
            >
              <span>← Volver a Métricas</span>
            </Link>
            <button
              onClick={() => loginAs(user?.role === 'ADMIN' ? 'EMPLOYEE' : 'ADMIN')}
              className="text-xs bg-gold/15 hover:bg-gold/25 border border-gold/30 text-gold font-bold px-3.5 py-2.5 rounded-full transition-all cursor-pointer"
            >
              Simular Rol: {user?.role || 'ADMIN'} ↺
            </button>
          </div>
        </div>
        ...
        {!online && (
          <div className="bg-amber-500 text-black text-[11px] font-extrabold uppercase px-3 py-1.5 rounded-full flex items-center justify-center gap-2 animate-fadeIn">
        ...
        {offlinePending > 0 && online && (
          <div className="bg-emerald-600 text-white text-[10px] font-bold px-3 py-1 rounded-full text-center">
